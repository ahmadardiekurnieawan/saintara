...
export default function AssessmentForm() {
  ...
  function validate() {
    ...
    else if (new Date(formData.birthDate) >= new Date()) e.birthDate = 'Tanggal lahir harus di masa lalu'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <Input
        ...
        hint="Gunakan nama lengkap sesuai dokumen resmi"
        autoComplete="name"
      />

      <div>
        <label className="block text-sm font-semibold text-primary mb-2">Tanggal Lahir</label>
        <div className="relative">
          <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted"><Calendar size={16} /></div>
          <input
            type="date"
            className={cn('input-field pl-11', errors.birthDate && 'border-red-400 focus:ring-red-400/30')}
            value={formData.birthDate}
            onChange={(e) => setFormData({ ...formData, birthDate: e.target.value })}
            max={new Date().toISOString().split('T')[0]}
          />
        </div>
        {errors.birthDate && <p className="mt-1.5 text-sm text-red-500">{errors.birthDate}</p>}
      </div>

      <div>
        <label className="block text-sm font-semibold text-primary mb-3">Golongan Darah</label>
        <div className="grid grid-cols-4 gap-2.5">
          {BLOOD_TYPES.map((type) => (
            <button
              ...
              className={cn(
                'py-3.5 rounded-xl border font-display font-bold text-lg transition-all duration-200',
        ...
        {errors.bloodType && <p className="mt-2 text-sm text-red-500">{errors.bloodType}</p>}
      </div>
